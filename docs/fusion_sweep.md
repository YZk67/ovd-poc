...
**结果（2026-09-19）：两份权重上没有任何门通过预算，这条线关闭。** 8ep 最好的门
`d0.25`：净救回 1 个 TP，多 1109 个 FP；kang 最好的 `d0.5`：净救回 10 个，多 1030 个。
CLIP 侧的门在两份权重上都是丢的 TP 远多于救回的（8ep 丢 13 到 144，kang 丢 107 到 227），
新增候选里 50% 到 60% 是背景框、25% 到 30% 是定位不到 0.5 的框：CLIP 的置信度不能当门，
它对背景和残缺框一样自信。更关键的是分区：8ep 有合格框却漏掉的 438 个 rare GT 里，
detector 单边能救 56、CLIP 单边能救 20、两边都救不了 362；kang 是 355 / 46 / 18 / 291。
重新加权现有分数最多只能碰到约 6% 的 rare GT，83% 的漏检需要分数本身改变，只能靠训练。
