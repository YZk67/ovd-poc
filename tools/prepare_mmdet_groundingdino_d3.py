...
NO_ADAPTER_CONTROL_TRAINABLE_PREFIXES = ("text_feat_map",)
...
def _make_config(args: argparse.Namespace, num_classes: int) -> str:
    ...
    if is_train and args.adapter_mode == "text":
        ...
        model_type = "    type='GroundingDINOD3PromptWrapper',\n"
    else:
        model_type = ""
    load_from = f"load_from = {_quote(args.checkpoint)}\n" if is_train else ""
    default_trainable_prefixes = (
        DEFAULT_TRAINABLE_PREFIXES
        if args.adapter_mode == "text"
        else NO_ADAPTER_CONTROL_TRAINABLE_PREFIXES
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Parameter prefix to leave trainable in training. Can be repeated. "
            "Defaults to text_query_adapter and dn_query_generator.label_embedding "
            "for --adapter-mode text, or text_feat_map for --adapter-mode none."
        ),
    )
    parser.add_argument(
        "--adapter-mode",
        choices=("text", "none"),
        default="text",
        help="'none' trains selected test-relevant parameters without adding the text adapter.",
    )
    ...
    manifest: Dict[str, Any] = {
        "purpose": (
            (
                ...
                if args.adapter_mode == "text"
                else "Route A no-adapter small-parameter control: MMDetection GroundingDINO-B/Swin-B on D3"
            )
            if is_train
