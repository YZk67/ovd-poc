...
The historical filename is retained; --target-epochs 12 selects continuous
4 -> 12ep training with an intermediate 8ep evaluation instead of an 8ep stop.
...
STOP_ITERATIONS = {8: 56800, 12: 85200}
LR_HORIZON = 85200
SNAPSHOT_NAME = "four_ep_snapshot"


def validate_checkpoint(checkpoint, *, target_epochs=8):
    """Require a real no-radius 4ep training checkpoint, not weights-only init."""
    if target_epochs not in STOP_ITERATIONS:
        raise ValueError("target_epochs must be 8 or 12; the LR horizon stays 85200.")
    ...
    return {"iteration": FOUR_EP_ITERATION, "resume_iteration": FOUR_EP_ITERATION + 1,
            "stop_iteration": STOP_ITERATIONS[target_epochs], "lr_scheduler_max_iter": LR_HORIZON,
...
def prepare(output_dir, *, snapshot=False, target_epochs=8):
    ...
    report = validate_checkpoint(checkpoint, target_epochs=target_epochs)
    del checkpoint
    ...
    if destination.exists():
        ...
        # A verified 4ep snapshot is independent of the *future* stopping point.
        # Reuse it when changing 8ep -> 12ep before starting, without rewriting
        # its original manifest or weakening any source identity checks.
        saved_resume = manifest["resume"]
        saved_identity = {k: v for k, v in saved_resume.items() if k != "stop_iteration"}
        current_identity = {k: v for k, v in report.items() if k != "stop_iteration"}
        if (saved_identity != current_identity
                or set(manifest["files"]) != {p.name for p in sources}):
        ...
        print(f"[ready] Resume with the {target_epochs}ep no-radius config and --resume; "
              f"stop={report['stop_iteration']}, LR horizon={LR_HORIZON}.", flush=True)
        return report
    ...
    print(f"[ready] Resume the SAME directory with the {target_epochs}ep no-radius config "
          f"and --resume; stop={report['stop_iteration']}, LR horizon={LR_HORIZON}.", flush=True)
    return report
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target-epochs", type=int, choices=sorted(STOP_ITERATIONS), default=8,
                        help="Planned final epoch (default: 8); does not change saved training state")
    parser.add_argument("--snapshot", action="store_true", help="Copy/verify 4ep evidence before first resume")
    args = parser.parse_args()
    prepare(args.output_dir, snapshot=args.snapshot, target_epochs=args.target_epochs)
